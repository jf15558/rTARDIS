#' get_grid
#'
#' Return all H3 cell IDs at a specified resolution within a rectangular
#' bounding box.
#'
#' @param bbox `numeric`. A vector of four numbers denoting the xmin, xmax, ymin
#' and ymax coordinates of the bounding box within which cells are to be
#' retrieved.
#' @param hex `integer`. The H3 resolution (1 to 15) at which to retrieve cells.
#' Note that retrieving high resolution grids (`hex > 4`) for large (i.e.,
#' approaching global) extents will take a long time and may become prohibitive.
#' @return A character vector of H3 cell IDs.
#' @import h3jsr
#' @export
#'
#' @details
#' The function can be viewed as the functional equivalent to a vector of raster
#' grid cell IDs at a specified resolution within that same bounding box. The
#' main difference is that the number of hexagonal cells in a given bounding box
#' cannot be easily calculated, unlike for a raster grid. The function
#' additionally controls for misbehaviour of bounding boxes approaching global
#' extents, avoiding subsequent complications for H3 cell retrieval.
#'
#' @examples
#' \donttest{
#' library(rTARDIS)
