...
#' or backwards in time regardless of the time directionality in the `tardis`
#' graph. Points without cells in their target layer will return `NA`.
#'
#' @param tardis `tardis`. The output of `build_tardis()`.
#' @param points `numeric` or `SpatVector`. Either a numeric vector of cell IDs
#' or a `SpatVector` of points with a cell ID variable named 'cell'.
...
#' `SpatVector` then a `SpatVector` of updated point geometries, with cell ID
#' (`$cell`) and time layer fields (`$layer`). For `NA` cells, a blank geometry
#' is returned.
...
time_shift <- function(tardis, points, layer, time = NULL) {
  ...
  for(i in 1:nrow(to_do)) {
    ...
    pt <- pts[which(pts[,3] == to_do[i,1] & pts[,4] == to_do[i,2]),2]
  ...
  if(inherits(points, "SpatVector")) {
    ...
    if(!is.na(tardis$gdat[7])) {
      ...
      points <- vect(cell_to_point(grd[pts[!is.na(pts[,2]),2] %% tardis$gdat[5]]))
    } else {
      samprast <- rast(nrows = tardis$gdat[5] / tardis$gdat[6], ncols = tardis$gdat[6],
                       ext = ext(tardis$gdat[1:4]))
      points <- vect(xyFromCell(samprast, pts[!is.na(pts[,2]),2]))
    }
    points$cell[!is.na(pts[,2])] <- pts[!is.na(pts[,2]),2]

    if(any(is.na(pts))) {

      points$id <- which(!is.na(pts[,2]))
      foo <- vect(matrix(nrow = sum(is.na(pts[,2])), ncol = 2))
      foo$cell <- NA
      foo$id <- which(is.na(pts[,2]))
