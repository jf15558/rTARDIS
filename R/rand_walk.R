#' rand_walk
#'
#' Conduct random walks within a `tardis` graph. Walk lengths can be specified
#' by the number of steps or a target distance to traverse. In the case of the
#' latter, the walk is terminated at the closest distance to the target as the
#' walks will not necessarily be able to precisely hit the target distance. Full
#' traverses in time and space are possible, but as walks in both these
#' dimensions not are not necessarily easy to interpret, so walks are restricted
#' to the time layer in which they originate as a default.
#'
...
#' starting points for the random walk(s).
#' @param mode `character`. The nature of the random walk, either the number of
#' `"steps"` to take or the `"cost"` in terms of the cumulative weight of the
#' edges traversed.
#' @param rwlen `numeric`. The length of the random walk as selected by `mode`.
#' Either a single number or a vector with as many elements as origin points,
#' enabling different walk lengths for each point. When `weights = NULL` and
#' `mode = "cost"`, this  can be directly interpreted as metres covered by the
#' random walk, but may also be expressed in relation to a user-supplied
#' weighting scheme.
#' @param restrict `logical`. Should the random walk should be constrained to
#' the same time layer as its origin. `TRUE` by default.
#' @param trials `integer`. The number of random walk trials to conduct before
#' quitting, if `mode = "cost"` and the random walk length proves insufficient
#' to reach the cost value.
#' @param verbose `logical`. Should function progress be reported to the user?
#' @return If `restrict = TRUE` (default), then a `SpatRaster` with each
#' layer recording the frequencies of cell visits. If `restrict = FALSE`,
#' a list of `SpatRaster` objects, each recording the frequencies of cells
#' visited by each random walk in each landscape layer through time.
#' @import terra h3jsr
